...
pub static COS_TABLE: LazyLock<[i16; 512]> = LazyLock::new(|| {
    std::array::from_fn(|i| {
        (250.0 * (2.0 * PI * i as f64 / 512.0).cos()).round() as i16
    })
});
...
pub static SIN_TABLE: LazyLock<[i16; 512]> = LazyLock::new(|| {
    std::array::from_fn(|i| {
        (250.0 * (2.0 * PI * i as f64 / 512.0).sin()).round() as i16
    })
});
...
const ANT_PAT_DB: [f64; 37] = [
     0.00,  0.00,  0.22,  0.44,  0.67,  1.11,  1.56,  2.22,
     3.10,  4.67,  6.89,  9.56, 12.78, 14.67, 15.56, 15.56, 15.56,
...
#[inline]
#[expect(clippy::indexing_slicing, reason = "idx_lo and idx_hi are clamped to [0, 36] — safe for the 37-element ANT_PAT_DB array")]
...
mod tests {
    ...
    #[test]
    #[expect(clippy::indexing_slicing, reason = "index 0 always exists in a 512-element array")]
    ...
    #[test]
    #[expect(clippy::indexing_slicing, reason = "loop iterates 0..512 matching the table size")]

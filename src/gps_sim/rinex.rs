...
use super::types::{Ephemeris, IonoUtc, UtcDate, GpsTime, consts::MAX_EPH_SETS};
...
#[expect(clippy::too_many_lines, reason = "RINEX parser necessarily handles many format variants in one pass")]
#[expect(clippy::while_let_loop, reason = "named break 'records inside inner loop prevents conversion to while-let")]
#[expect(clippy::manual_let_else, reason = "break/continue targets prevent let-else refactoring here")]
#[expect(clippy::field_reassign_with_default, reason = "Ephemeris is populated field-by-field from parsed arrays; struct literal would be unwieldy")]
#[expect(clippy::indexing_slicing, reason = "fields[n] guarded by nf<28; current_set[prn-1] guarded by prn 1..=32; nums indexed after len check")]
fn parse(reader: Box<dyn BufRead>) -> Result<NavData, SimError> {
    ...
    loop {
            ...
            || (label.starts_with("TIME SYSTEM CORR") && line.contains("GPUT"))
        {
            ...
            iono.tot = line.get(off + 38..off + 57).unwrap_or("").trim().parse().unwrap_or(0);
            iono.wnt = line.get(off + 57..).unwrap_or("").trim().parse().unwrap_or(0);
            flags |= 0x4;
    ...
    'records: loop {
        ...
        let (prn, col0): (usize, usize) = if version >= 3.0 {
            ...
            if !line.starts_with('G') {
                for _ in 0..7 { let _skip: io::Result<String> = read_line(&mut lines); }
                continue;
            }
        ...
        if nf < 24 { continue; } // incomplete record — skip
        ...
        e.toe       = GpsTime { week: fields[18] as i32, sec: fields[8] };
        e.cic       = fields[9];
        e.omg0      = fields[10];
        e.cis       = fields[11];
        e.inc0      = fields[12];
        ...
        if dt > 3_600.0 {
            if g0.week >= 0 {
                ...
                set_count += 1;
                if set_count >= MAX_EPH_SETS { break; }
            }
            g0 = t;
    ...
    Ok(NavData { count: set_count, eph: eph_sets, iono })
}
...
fn parse_f64_field(line: &str, off: usize, w: usize) -> f64 {
    line.get(off..off + w)
        .map(|s| s.trim().replace(['D', 'd'], "E").parse::<f64>().unwrap_or(0.0))
        .unwrap_or(0.0)
}
...
#[expect(clippy::indexing_slicing, reason = "nums indexed 0..5; guarded by len() >= 6 check above")]
fn parse_epoch(line: &str, col0: usize) -> Result<UtcDate, SimError> {
    ...
    let nums: Vec<f64> = s.split_whitespace()
        ...
        .collect();
    if nums.len() < 6 {
        return Err(SimError::Rinex(format!("bad epoch in: '{}'", &line[..line.len().min(40)])));
    }
    // Two-digit year: ≥80 → 19xx, <80 → 20xx (RINEX 2 convention).
    let year = {
        let y = nums[0] as i32;
        if y >= 80 { y + 1900 } else if y < 100 { y + 2000 } else { y }
    };
    Ok(UtcDate {
        year,
        month: nums[1] as u8,
